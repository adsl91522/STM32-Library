...
{
	uint8_t	ret;
	HAL_SPI_TransmitReceive(&W25QXX_SPI, &Data, &ret, 1, W25QXX_TIMEOUT_VALUE);
	return ret;	
}
//###################################################################################################################
uint32_t W25qxx_ReadID(void)
{
	...
	W25QXX_CS(0);
	W25qxx_Spi(0x9F);
	Temp0 = W25qxx_Spi(0x00);					//MSB
	Temp1 = W25qxx_Spi(0x00);
	Temp2 = W25qxx_Spi(0x00);
	W25QXX_CS(1);
...
bool W25qxx_IsEmptyPage(uint32_t Page_Address,uint32_t OffsetInByte,uint32_t NumByteToCheck_up_to_PageSize)
{
	...
	for(i=OffsetInByte; i<w25qxx.PageSize; i+=sizeof(pBuffer))
	{
		...
		W25qxx_Spi(0);
		HAL_SPI_Receive(&W25QXX_SPI, pBuffer,sizeof(pBuffer),100);
		W25QXX_CS(1);
	...
	if((w25qxx.PageSize+OffsetInByte)%sizeof(pBuffer)!=0)
	{
		...
		for( ; i<w25qxx.PageSize; i++)
		{
			...
			W25qxx_Spi(WorkAddress & 0xFF);
			W25qxx_Spi(0);
			HAL_SPI_Receive(&W25QXX_SPI, pBuffer, 1, 100);
			W25QXX_CS(1);
			if(pBuffer[0]!=0xFF)
				goto NOT_EMPTY;
		}
...
void W25qxx_WritePage(uint8_t *pBuffer, uint32_t Page_Address, uint32_t OffsetInByte, uint32_t NumByteToWrite_up_to_PageSize)
{
	...
	HAL_SPI_Transmit(&W25QXX_SPI, pBuffer, NumByteToWrite_up_to_PageSize, 100);
	W25QXX_CS(1);
	W25qxx_WaitForWriteEnd();
#if (W25QXX_DEBUG==1)
...
void W25qxx_ReadBytes(uint8_t* pBuffer, uint32_t ReadAddr, uint32_t NumByteToRead)
{
	...
	W25qxx_Spi(0);
	HAL_SPI_Receive(&W25QXX_SPI, pBuffer, NumByteToRead, W25QXX_TIMEOUT_VALUE);
	W25QXX_CS(1);
#if (W25QXX_DEBUG==1)
	StartTime = HAL_GetTick()-StartTime; 
...
void  W25qxx_ReadPage(uint8_t *pBuffer,uint32_t Page_Address, uint32_t OffsetInByte, uint32_t NumByteToRead_up_to_PageSize)
{
	...
	W25qxx_Spi(Page_Address & 0xFF);
	W25qxx_Spi(0);
	HAL_SPI_Receive(&W25QXX_SPI, pBuffer, NumByteToRead_up_to_PageSize, W25QXX_TIMEOUT_VALUE);
	W25QXX_CS(1);
#if (W25QXX_DEBUG==1)
	StartTime = HAL_GetTick()-StartTime; 
